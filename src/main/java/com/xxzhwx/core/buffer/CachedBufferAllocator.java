...
public class CachedBufferAllocator implements IoBufferAllocator {
   ...
   *          the maximum capacity of a cached buffer. A buffer whose capacity
   *          is bigger than this value is not pooled. <tt>0</tt> disables this
   *          limitation.
   */
  ...
  /**
   * Returns the maximum capacity of a cached buffer. A buffer whose capacity is
   * bigger than this value is not pooled. <tt>0</tt> means 'no limitation'.
   */
    ...
    private void free(ByteBuffer oldBuf) {
      if ((oldBuf == null)
          || ((maxCachedBufferSize != 0)
              && (oldBuf.capacity() > maxCachedBufferSize))
