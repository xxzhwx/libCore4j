 ...
 *
 */
package com.xxzhwx.core.buffer;
...
public abstract class IoBuffer implements Comparable<IoBuffer> {
   ...
   * default when the type of the new buffer is not specified. The default value
   * is <tt>false</tt>.
   */
   ...
   *          the capacity of the buffer
   * @param direct
   *          <tt>true</tt> to get a direct buffer, <tt>false</tt> to get a heap
   *          buffer.
   */
   ...
   * that it can be reused by some {@link IoBufferAllocator} implementations. It
   * is not mandatory to call this method, but you might want to invoke this
  ...
  /**
   * Returns the minimum capacity of this buffer which is used to determine the
   ...
   * equal to the current capacity, this method returns silently. If the new
   * capacity is greater than the current capacity, the buffer is reallocated
   * while retaining the position, limit, mark and the content of the buffer.
   */
  ...
  /**
   * Changes the capacity of this buffer so this buffer occupies as less memory
   * as possible while retaining the position, limit and the buffer content
   * between the position and limit. The capacity of the buffer never becomes
   * less than {@link #minimumCapacity()}. The mark is discarded once the
   * capacity changes.
   */
   ...
   * <code>encoder</code>. This method doesn't terminate string with
   ...
   */
  public abstract IoBuffer putPrefixedString(CharSequence in, int prefixLength,
      int padding, CharsetEncoder encoder) throws CharacterCodingException;
   ...
   * length as a prefix and the buffer has remaining data as enough as specified
   * in the data length field. This method is identical with
   * <tt>prefixedDataAvailable( prefixLength, Integer.MAX_VALUE )</tt>. Please
   * not that using this method can allow DoS (Denial of Service) attack in case
   * the remote peer sends too big data length value. It is recommended to use
  ...
  /**
   * Returns the first occurence position of the specified byte from the current
